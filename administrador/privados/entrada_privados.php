...
if ($_GET['remover']) {
    ...
    if($_GET["id_entrada"] > 0){
        $mesa = $dbprivados->devolveMesa($_GET['id_mesa']);
        if ($mesa) {
            $query = 'DELETE FROM privados_salas_mesas_entradas WHERE id = "' . $_GET["id_entrada"] . '" AND data_evento= "' . $data_evento . '" AND id_mesa="' . $_GET['id_mesa'] . '"';
            $db->query($query);
            $estado = $db->Insert('logs', array('descricao' => "Apagou uma entrada de privados para a mesa ID " . $_GET['id_mesa'], 'arr' => json_encode($campos), 'id_admin' => $_SESSION['id_utilizador'], 'tipo' => "Apagar", 'user_agent' => $_SERVER['HTTP_USER_AGENT'], 'ip' => $_SERVER['REMOTE_ADDR']));
            $_SESSION['sucesso'] = "Entrada removido com sucesso.";
        } else {
                            ...
                            <div class="mesas">
                                ...
                                ?>
                                    <div class="mesa">
                                        ...
                                        if ($ocupada) {
                                            $entradas = $dbprivados->devolveEntradasMesa($mesa['id'], $data_evento);
                                        ?>
                                            <div class="info_reserva">
                                                ...
                                                <div class="botoes">
                                                    <a href="?pg=entrada_privados&id_mesa=<?php echo $mesa['id']; ?>&remover=1" class="cancelar libertar">
                                                        Libertar mesa
                                                    </a>
                                                    <a href="?pg=entrada_privados&id_mesa=<?php echo $mesa['id']; ?>&adicionar=1" data-id="<?php echo $mesa['id']; ?>" class="cancelar adicionar">
                                                        Adicionar entrada
                                                    </a>
                                                </div>
                                                <?php
                                                foreach ($entradas as $key => $value) {
                                                   ...
                                                   <div class="entrada">
                                                       ...
                                                       <div class="bloco">
                                                            ...
                                                            <span class="valor">
                                                                <?php echo $value["cartoes"]; ?>
                                                            </span>
                                                       </div>
                                                        <div class="botoes">
                                                            <a href="?pg=entrada_privados&id_entrada=<?php echo $value["id"]; ?>&id_mesa=<?php echo $mesa['id']; ?>&remover=1" class="cancelar apagar_entrada">
                                                                Apagar
                                                            </a>
                                                            <a href="/administrador/privados/imprimir_entrada_privados.php?id=<?php echo $value["id"]; ?>" target="_blank" class="cancelar imprimir_entrada">
                                                                Imprimir
                                                            </a>
                                                        </div>
                                                   </div>
